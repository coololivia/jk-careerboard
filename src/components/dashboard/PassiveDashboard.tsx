  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-jk-text-muted">조건 맞는 공고 나오면 바로 알려드릴게요</p>
      </div>
      ...
      {!(data as any).pushNotificationConsent && (
        <section className="rounded-[16px] bg-jk-orange p-4 text-white">
          ...
          <p className="mt-1 text-xs opacity-80">조건 맞는 공고 올라오면 카카오 알림톡으로 바로 알려드려요</p>
          <button className="mt-3 w-full rounded-[10px] bg-white py-2 text-sm font-bold text-jk-orange">
            알림 받기
          </button>
        </section>
      )}
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-xs text-jk-text-muted">{(matchedJobs ?? []).length}개</span>
        </div>
