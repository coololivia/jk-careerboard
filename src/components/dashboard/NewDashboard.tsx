  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-jk-text-muted">첫 취업까지 함께할게요</p>
      </div>
      ...
      <section className="rounded-[16px] border border-jk-border bg-white p-4">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-base font-semibold text-jk-text-strong">이력서 완성도</h2>
          <span className="text-sm font-bold text-jk-orange">{user.resumeCompleteness}%</span>
        </div>
        <div className="h-2 rounded-full bg-jk-bg-section">
          <div
            className="h-2 rounded-full bg-jk-orange transition-all"
          ...
          {(resumeProgress?.missingItems ?? []).map((item: any) => (
            <div key={item.field} className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-jk-border" />
                <p className="text-sm text-jk-text-secondary">{item.label}</p>
                {item.required && <span className="text-xs text-red-500">필수</span>}
              </div>
              <button className="text-xs font-medium text-jk-orange">추가하기</button>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* 맞춤 공고 */}
      <section>
        <h2 className="mb-3 text-base font-semibold text-jk-text-strong">지금 지원 가능한 공고</h2>
