...
function CareerTimeline({ timeline }: { timeline: TimelineEvent[] }) {
  return (
    <div className="mt-4 border-t border-gray-100 pt-4">
      <p className="mb-3 text-xs font-medium text-gray-500">커리어 성장 타임라인</p>
        ...
        {timeline.map((item, i) => (
          <div key={i} className="flex gap-3">
            ...
            <div className="flex flex-col items-center">
              ...
              />
              {i < timeline.length - 1 && (
                <div className="my-0.5 w-px flex-1 bg-gray-200" style={{ minHeight: 20 }} />
              )}
            </div>
            {/* 내용 */}
            <div className="pb-4">
              ...
              <p className="mt-0.5 text-sm font-semibold text-gray-800">{item.event}</p>
...
export default function CareerScoreCard({ data, timeline, userType }: Props) {
  ...
  // 신입: 격려 프레이밍 (NewDashboard에 이미 있지만 공용 컴포넌트로도 지원)
  if (userType === "new") {
    return (
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-2xl font-bold">{data.score}점</p>
          </div>
          <div className="text-right">
            <p className="text-sm font-semibold text-orange-500">{data.salaryVsMarket}</p>
            <p className="mt-0.5 text-xs text-gray-400">시장 평균 대비</p>
          </div>
        </div>
      </section>
    );
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-0.5 flex items-end gap-1.5">
            <p className="text-3xl font-bold">{data.score}점</p>
            <span className={`mb-0.5 text-sm font-semibold ${trendColor}`}>
              {trendArrow} +{data.trendDelta}점
            </span>
          </div>
          <p className="mt-1 text-xs font-medium text-blue-600">{data.marketComparison}</p>
        </div>
      ...
      <div className="mt-3">
        <div className="mb-1 flex items-center justify-between">
          <p className="text-xs text-gray-400">시장 내 위치</p>
          <p className="text-xs font-medium text-gray-600">상위 {data.percentile}%</p>
        </div>
        <div className="relative h-2 rounded-full bg-gray-100">
          ...
          >
            <div className="h-3.5 w-3.5 rounded-full border-2 border-blue-600 bg-white shadow-sm" />
          </div>
        </div>
      ...
      {timeline && timeline.length > 0 && (
        <>
          <button
            ...
            className="mt-3 flex w-full items-center justify-between text-xs text-gray-500"
          >
